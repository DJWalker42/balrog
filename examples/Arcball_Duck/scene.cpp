...
/*	Note: These function are implemented for a simple example scene were we only have one shader program for the scene. 
	In "real" applications there are likely to be multiple shader programs in use; the scene writer can make these 
	virtual functions as complex as they like for their particular application. In this case it would be nice to provide
	some mechanism by which we can refer to specific shaders e.g. a string literal id, rather than
	remembering where in the container (m_shaderList) the shader is located.
*/
...
void myScene::initScene(GLFWwindow* window)
{
	...
	//In our example shaders their are two structures. The first, called Light, has position and intensity data members,
	//and the second, called Material, has coefficients for diffuse, specular, and ambient reflection properties, as 
	//well as a "shinyness" factor (Phong model of shading).
	...
	//There is also a boolean value that can use this to switch between Phong and Oren-Nayer shading models.
	...
	//if we add more textures to a renderable then we can add their names here e.g. mixMap.
